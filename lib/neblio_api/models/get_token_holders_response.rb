...
module NeblioAPI
  class GetTokenHoldersResponse
    ...
    def self.openapi_types
      {
        :'token_id' => :'String',
        :'holders' => :'Array<GetTokenHoldersResponseHolders>',
        :'divibility' => :'Float',
        :'lock_status' => :'BOOLEAN',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NeblioAPI::GetTokenHoldersResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NeblioAPI::GetTokenHoldersResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'divibility')
        self.divibility = attributes[:'divibility']
      end
    ...
    def hash
      [token_id, holders, divibility, lock_status, aggregation_policy, some_utxo].hash
    end
